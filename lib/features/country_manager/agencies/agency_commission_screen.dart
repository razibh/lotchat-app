...
class _AgencyCommissionScreenState extends State<AgencyCommissionScreen> with SingleTickerProviderStateMixin {
  ...
  List<HostCommission> _generateHostCommissions(int count) {
    final var hosts = <HostCommission><HostCommission>[];
  ...
  List<CommissionTransaction> _generateTransactions(int count) {
    final var transactions = <CommissionTransaction><CommissionTransaction>[];
    for (var i = 0; i < count; i++) {
  ...
  void _calculateStats() {
    ...
    _averageCommissionRate = _hostCommissions.isEmpty 
        ? 0 
        : _hostCommissions.fold(0, (double sum, HostCommission h) => sum + h.commissionRate) / _hostCommissions.length;
  }
  ...
  Widget _buildHostSummary() {
    final var totalHostEarnings = _hostCommissions.fold(0, (double sum, HostCommission h) => sum + h.monthlyEarnings);
    var totalHostCommission = _hostCommissions.fold(0, (double sum, HostCommission h) => sum + h.commissionAmount);
  ...
  Widget _buildHostCommissionTile(HostCommission host) {
    var statusColor = host.status == 'Active' ? Colors.green : 
  ...
  Widget _buildTransactionTile(CommissionTransaction transaction) {
    var typeColor = transaction.type == 'Commission' ? Colors.green :
                      transaction.type == 'Withdrawal' ? Colors.blue : Colors.orange;
    var statusColor = transaction.status == 'Completed' ? Colors.green : Colors.orange;
  ...
  void _showAdjustRateDialog(HostCommission host) {
    final rateController = TextEditingController(text: host.commissionRate.toString());
