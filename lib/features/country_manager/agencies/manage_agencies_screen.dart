...
class CountryManagerAgenciesScreen extends StatefulWidget {
  ...
  const CountryManagerAgenciesScreen({
    required this.managerId, required this.countryId, required this.countryName, super.key,
  });
...
class _CountryManagerAgenciesScreenState extends State<CountryManagerAgenciesScreen> with SingleTickerProviderStateMixin {
  ...
  List<String> _getCurrentCities() {
    if (_selectedCountry == 'All Countries') {
      ...
      final allCities = <String><String><String>{'All Cities'};
  ...
  List<ManagerAgency> _generateSampleAgenciesForCountry(Map<String, String> country, int count) {
    final var agencies = <ManagerAgency><ManagerAgency><dynamic>[];
    final var countryId = country['id']!;
    final var countryName = country['name']!;
    final var cities = _countryCities[countryId]?.where((String c) => c != 'All Cities').toList() ?? <String>[];
  ...
  void _calculateStats() {
    ...
    _averageGrowth = _agencies.fold(0, (int sum, ManagerAgency a) => sum + a.monthlyGrowth) / _agencies.length;
  ...
  Widget _buildAgencyCard(ManagerAgency agency) {
    ...
    final MaterialColor riskColor = agency.riskLevel == 'High' ? Colors.red :
                      agency.riskLevel == 'Medium' ? Colors.orange : Colors.green;

    final bool needsMeeting = agency.nextMeeting != null && 
        ...
        content: Column(
          ...
          children: <>[
            ...
            ListTile(
              ...
              onTap: () async {
                final var TimeOfDay? picked = await showTimePicker(
                  context: context,
...
class ManagerAgency {
  ...
  ManagerAgency({
    ...
    required this.taxId,
    required this.performanceScore, required this.riskLevel, this.website,
