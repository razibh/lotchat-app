...
class ParticipantGrid extends StatelessWidget {
  ...
  const ParticipantGrid({
    required this.participants, super.key,
    this.localUserId,
  ...
  void debugFillProperties(DiagnosticPropertiesBuilder properties) {
    ...
    properties.add(IterableProperty<Participant>('participants', participants));
    properties.add(StringProperty('localUserId', localUserId));
    properties.add(ObjectFlagProperty<Function(String userId)?>.has('onParticipantTap', onParticipantTap));
    properties.add(ObjectFlagProperty<Function(String userId)?>.has('onMuteParticipant', onMuteParticipant));
    properties.add(ObjectFlagProperty<Function(String userId)?>.has('onPinParticipant', onPinParticipant));
  }
}
...
class ParticipantTile extends StatelessWidget {
  ...
  const ParticipantTile({
    required this.participant, required this.isLocal, super.key,
    this.onTap,
    this.onMute,
    this.onPin,
  });
  ...
  void debugFillProperties(DiagnosticPropertiesBuilder properties) {
    super.debugFillProperties(properties);
    properties.add(DiagnosticsProperty<Participant>('participant', participant));
...
class AudioLevelPainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    final double barWidth = size.width / 3;
    const int barSpacing = 2;

    for (var i = 0; i < 3; i++) {
      final double barHeight = size.height * (0.3 + (level * 0.7) * (i + 1) / 3);
