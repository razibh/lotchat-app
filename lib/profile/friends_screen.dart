...
class _FriendsScreenState extends State<FriendsScreen> with DialogMixin {
  ...
  void _applyFilter() {
    var filtered = List<UserModel>.from(_friends);

    // Apply search
    if (_searchQuery.isNotEmpty) {
      filtered = filtered.where((Object? f) {
    ...
    switch (_selectedFilter) {
      case 'Online':
        filtered = filtered.where((Object? f) => f.isOnline).toList();
      case 'Favorites':
        filtered = filtered.where((Object? f) => f.stats['favorite'] == true).toList();
      case 'Recent':
        filtered.sort((Object? a, Object? b) => b.lastActive.compareTo(a.lastActive));
    }
  ...
  Future<void> _unfriend(UserModel friend) async {
    ...
    if (confirmed == true) {
      try {
        ...
        setState(() {
          _friends.removeWhere((Object? f) => f.uid == friend.uid);
          _applyFilter();
        });
