...
class RoomService {
  ...
  Stream<RoomModel?> streamRoom(String roomId) {
    return _firestore
        ...
        .doc(roomId)
        .snapshots()
        .map((doc) => doc.exists ? RoomModel.fromJson(doc.data()!) : null);
  }
  ...
  Future<List<RoomModel>> getRecommendedRooms(String userId, {int limit = 10}) async {
    try {
      ...
      final visitedRoomIds = historySnapshot.docs
      ...
      final rooms = roomsSnapshot.docs
          .map((doc) => RoomModel.fromJson(doc.data()))
          .where((r) => !visitedRoomIds.contains(r.id))
          .toList();

      // Score rooms
      final scoredRooms = rooms.map((room) {
        double score = 0;
      ...
      return scoredRooms.take(limit).map((item) => item['room'] as RoomModel).toList();
    } catch (e) {
