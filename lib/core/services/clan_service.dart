...
class ClanService {
  ...
  Future<ClanModel?> createClan({
    ...
    try {
      ...
      final DocumentReference<Map<String, dynamic>> docRef = await _firestore.collection('clans').add(clanData.toJson());
  ...
  Future<ClanModel?> getClan(String clanId) async {
    try {
      final DocumentSnapshot<Map<String, dynamic>> doc = await _firestore.collection('clans').doc(clanId).get();
      if (doc.exists) {
  ...
  Stream<ClanModel?> streamClan(String clanId) {
    return _firestore
        ...
        .doc(clanId)
        .snapshots()
        .map((DocumentSnapshot<Map<String, dynamic>> doc) => doc.exists ? ClanModel.fromJson(doc.data()!) : null);
  }
  ...
  Future<List<ClanModel>> searchClans(String query) async {
    try {
      ...
      return snapshot.docs
          .map((QueryDocumentSnapshot<Map<String, dynamic>> doc) => ClanModel.fromJson(doc.data()))
          .toList();
    } catch (e) {
  ...
  Future<List<ClanModel>> getRecommendedClans(String userId, {int limit = 10}) async {
    try {
      ...
      final clans = snapshot.docs
          .map((QueryDocumentSnapshot<Map<String, dynamic>> doc) => ClanModel.fromJson(doc.data()))
          .where((Object? c) => !c.members.any((m) => m.userId == userId))
          .toList();

      // Score and sort
      clans.sort((Object? a, Object? b) {
        final int scoreA = _calculateClanScore(a, user);
        final int scoreB = _calculateClanScore(b, user);
  ...
  Future<bool> joinClan(String clanId) async {
    ...
    try {
      final DocumentReference<Map<String, dynamic>> clanRef = _firestore.collection('clans').doc(clanId);
      
      return await _firestore.runTransaction((Transaction transaction) async {
        final DocumentSnapshot<Map<String, dynamic>> clanDoc = await transaction.get(clanRef);
  ...
  Future<bool> approveRequest(String requestId, String clanId) async {
    ...
    try {
      final DocumentReference<Map<String, dynamic>> requestRef = _firestore.collection('clan_requests').doc(requestId);
      final DocumentSnapshot<Map<String, dynamic>> requestDoc = await requestRef.get();
      ...
      return await _firestore.runTransaction((Transaction transaction) async {
        ...
        // Get user data
        final DocumentSnapshot<Map<String, dynamic>> userDoc = await transaction.get(
  ...
  Stream<List<Map<String, dynamic>>> getActivityLogs(String clanId, {int limit = 50}) {
    return _firestore
        ...
        .limit(limit)
        .snapshots()
        .map((QuerySnapshot<Map<String, dynamic>> snapshot) => snapshot.docs.map((QueryDocumentSnapshot<Map<String, dynamic>> doc) => doc.data()).toList());
  }
