...
class SellerService {
  ...
  Stream<Seller?> streamSeller(String sellerId) {
    return _firestore
        ...
        .doc(sellerId)
        .snapshots()
        .map((doc) => doc.exists ? Seller.fromJson(doc.data()!, doc.id) : null);
  }
  ...
  Future<SellerStats> getSellerStats(String sellerId) async {
    ...
    final todayCoins = todaySales.docs.fold<int>(
      0,
      (sum, doc) => sum + (doc.data()['coins'] ?? 0),
    );
    
    final todayEarnings = todaySales.docs.fold<double>(
      0,
    ...
    final monthSales = await _firestore
        .collection('coin_sales')
        .where('sellerId', isEqualTo: sellerId)
        .where('timestamp', isGreaterThanOrEqualTo: startOfMonth)
        .get();
    
    final monthCoins = monthSales.docs.fold<int>(
      0,
      (sum, doc) => sum + (doc.data()['coins'] ?? 0),
    );
    
    final monthEarnings = monthSales.docs.fold<double>(
      0,
      (sum, doc) => sum + (doc.data()['amount'] ?? 0),
    );
  ...
  Stream<List<CoinSale>> getSalesHistory(String sellerId, {int limit = 50}) {
    return _firestore
        ...
        .map((snapshot) => snapshot.docs
            .map((doc) => CoinSale.fromJson(doc.data(), doc.id))
            .toList());
  }
  ...
  Future<void> requestRestock(int amount) async {
    ...
    final seller = await getSeller(currentUser.uid);
    if (seller == null) throw Exception('Seller not found');
