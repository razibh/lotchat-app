...
class ModerationService {
  ...
  ModerationResult moderateText(String text) {
    ...
    final foundBannedWords = <String><String><dynamic>[];
  ...
  Stream<List<Report>> getReports({ReportStatus? status}) {
    Query<Map<String, dynamic>> query = _firestore.collection('reports').orderBy('timestamp', descending: true);
    ...
    return query.snapshots().map((QuerySnapshot<Map<String, dynamic>> snapshot) => snapshot.docs
        .map((QueryDocumentSnapshot<Map<String, dynamic>> doc) => Report.fromJson(doc.data(), doc.id))
        .toList(),);
  }
...
class Report {
  ...
  Report({
    ...
    required this.status, required this.timestamp, this.description,
    this.evidence,
